...
def analyze_codebase(files: list[SourceFile]) -> AnalysisResult:
    findings: list[Finding] = []
    for source_file in files:
        findings.extend(_analyze_single_file(source_file))
    return AnalysisResult(findings=findings, risk_score=_compute_risk_score(findings, len(files)))


def _analyze_single_file(source_file: SourceFile) -> list[Finding]:
    findings: list[Finding] = []
    lines = source_file.content.splitlines()

    findings.extend(_detect_db_calls_inside_loops(source_file.relative_path, lines))
    findings.extend(_detect_blocking_calls(source_file.relative_path, lines))
    findings.extend(_detect_nested_loops(source_file.relative_path, lines))
    findings.extend(_detect_unbounded_accumulation(source_file.relative_path, lines))
    findings.extend(_detect_maintainability_risk(source_file.relative_path, lines))
    findings.extend(_detect_broad_exception_swallowing(source_file.relative_path, lines))
    findings.extend(_detect_missing_timeouts(source_file.relative_path, lines))
...
def _detect_missing_timeouts(file_path: str, lines: list[str]) -> list[Finding]:
    findings: list[Finding] = []
    for idx, line in enumerate(lines):
        if REQUEST_CALL_PATTERN.search(line) or FETCH_PATTERN.search(line) or AXIOS_PATTERN.search(line):
